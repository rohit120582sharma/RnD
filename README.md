# Partner Portal
UI Client for Partner Portal Application
...
Install Homebrew
```
Open terminal and run below commands step by step:
```sh
...
brew update
```
Then run brew command to install Java library.
```sh
brew install ant
```
Next, add Homebrew's location to your $PATH inyour `.bash_profile` or `.zshrc` file.
```sh
...
```
#### Install webpack, compass
```sh
sudo npm install -g webpack
sudo gem install compass
```
#### Check/Confirm installation
```sh
node -v && npm -v && webpack -V && yarn -v && ant -version && java -version && ruby -v
```
...
## Setup Git Repository Locally : Git clone
```sh
git clone https://git.autodesk.com/ACG-Cloud-Services/dmg-partner-portal-server.git
```
...
## Dependencies and build up
#### Install dependecines to create the build
```sh
cd public
yarn install
```

#### Command to serve the application in browser with live-reloading
```sh
yarn start
```
...
> 'yarn' would generate final deployable code, which would be then hosted on your machine via Apache Server.
...
<VirtualHost local-partner-portal.dmg.autodesk.com:443>
    ServerAdmin local@local-partner-portal.dmg.autodesk.com
    DocumentRoot "/Users/<user>/Work/dmg-partner-portal-server/partner-portal/public"
    ServerName local-partner-portal.dmg.autodesk.com
    ServerAlias local-partner-portal.dmg.autodesk.com
    AllowEncodedSlashes NoDecode
    SSLEngine on
    SSLCertificateFile "conf/ssl.crt/server.crt"
    SSLCertificateKeyFile "conf/ssl.key/server.key"
    <Directory "/Users/<user>/Work/dmg-partner-portal-server/partner-portal">
        Options Indexes FollowSymLinks Includes ExecCGI
        AllowOverride All
        Require all granted
    </Directory>
    ErrorLog "/var/log/apache2/local-partner-portal.dmg.autodesk.com-error.log"
    CustomLog "/var/log/apache2/local-partner-portal.dmg.autodesk.com.log" common
</VirtualHost>
```
...
```sh
sudo nano /etc/hosts
127.0.0.1 local-partner-portal.dmg.autodesk.com
```
...
- search for line starting with 'LoadModule php', and if it is commented then please uncomment it. For e.g., 
\#LoadModule php7_module libexec/apache2/libphp7.so --> LoadModule php7_module libexec/apache2/libphp7.so

- similarily, search for line starting with 'LoadModule rewrite', and if it is commented then please uncomment it. For e.g.. 
...
Now, we would need to restart the apache server for changes in configuration to take effect. 
```sh
sudo apachectl restart
```

### Load the Website in browser
Open URL https://local-partner-portal.dmg.autodesk.com/ and it should take you to the Learn It Home Page, if you are already logged in to the alpha oxygen environment. If not, it will redirect you to Login Page, from wherein upon successful credentials entry, would callback to the Learn It Home Page.
...
### 1. Installing HomeBrew on your Machine 

If brew command is not recognized, i.e. HomeBrew is not installed on your machine, then you can set it up using below script

```sh
ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"
```
### 2. Installing Composer (In case encountered error at step 3)
Composer Would require to be installed in your machine in case it's not already available. 
You can use below steps on MAC, to install 'Composer' via brew in case it's not available.
...
# Create React APP - Boiler Plate Related User Guide
